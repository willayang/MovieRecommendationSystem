...
def error_input(cause, data):
  if cause == 1:
    ...
    print 'data type of line' + data +'is wrong'
    os._exit()

def check_rating(users, movies, ratins):
    usertomovie = {}
    if users not in usertomoive:
        usertomoive[users] = len(usertomoive)
    elif usertomovie[users] == movies:
        error_input(2, line)
        return False
    return True
...
def construct_rating_matrix():
  ...
  for line in sys.stdin:
    ...
    if float(fields[0]) != int(fields[0]): ###
       error_input(3, line)
    if float(fields[1]) != int(fields[1]): ###
       error_input(3, line)
    ####
...
def construct_co_occurence(all_ratings):
  num_movies = np.shape(all_ratings)[0]
